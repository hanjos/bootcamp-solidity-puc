...
contract('Funds: Investing phase', function (accounts) {
  ...
  it("can't receive money from just anybody", async function () {
    ...
    // XXX this is an issue with number precision in Javascript; strictly
    //     speaking, the owner should end with the same balance as before,
    //     minus gas costs (which has a little can of worms of its own); but
    //     since Javascript doesn't have native arbitrary-precision arithmetic,
    //     I'll need to find a library. For now, the "* 2" should work
